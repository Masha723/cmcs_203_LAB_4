...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public void setValues( int value, int row )
   {
    ...
    //  and column is the index of the array's current column
    // Write your code here:
	   int len=intArray[row].length;
	   for (int index=0;index<len;index++) {
		   intArray[row][index]=value;
		   animate(row,index);
	   }
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   ...
	   int smallest=intArray[0][column];
	   
	   for (int index=0;index<len;index++)
	   {
		   if (intArray[index][column]<smallest) 
		   {
			   smallest=intArray[index][column];
		   }
		   animate(index,column,smallest);
		   
		   
	   }

      return smallest; // replace this line with your return statement
   }
   ...
   */
   public int countFound( int value )
   {
    ...
    // Write your code here:
	   ...
	   for (int index=0;index<len;index++) 
	   {
		   len_column=intArray[index].length;
		   for (int i=0;i<len_column;i++) 
		   {
			   int val=intArray[index][i];
			   if (value==val)
			   {
				   count++;
			   }
			   animate(index,i,count);
		   }
	   }
